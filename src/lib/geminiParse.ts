...
export const CATEGORIES = ['Receita','Groceries','Restauração','Compras','Saúde','Transportes','Lazer','Levantamentos','Habitação','Utilities','Subscrições','Investimentos','Comissões e Taxas','Transferências','Despesas Gerais']
...
export type ParsedTransaction = { data: string; descritivo: string; valor: number; categoria: string }
export type ParsedMeta = { saldo_final: number | null; iban: string | null; numero_conta: string | null; periodo_fim: string | null }
export type ParseResult =
  | { ok: true; transactions: ParsedTransaction[]; meta: ParsedMeta }
  | { ok: false; error: string; status: number }

// Recebe os bytes de um ficheiro (PDF/Excel/CSV) já em base64 + mimeType, chama o Gemini,
// e devolve transações + metadados estruturados. Usado tanto pelo upload directo como pelo import via Drive.
export async function parseStatementWithGemini(base64: string, mimeType: string, sizeMB: number): Promise<ParseResult> {
  const apiKey = process.env.GEMINI_API_KEY
  if (!apiKey) return { ok: false, error: 'GEMINI_API_KEY não configurada', status: 500 }
  ...
  if (!geminiRes.ok) {
    ...
    if (geminiRes.status === 429) return { ok: false, error: 'Limite da API atingido. Aguarda alguns segundos e tenta de novo.', status: 429 }
    if (geminiRes.status === 413 || sizeMB > 18) return { ok: false, error: `Ficheiro demasiado grande (${sizeMB.toFixed(1)}MB). Tenta dividir o PDF em partes mais pequenas.`, status: 413 }
    return { ok: false, error: `Erro Gemini: ${geminiRes.status}. ${errText.slice(0,200)}`, status: 500 }
  }
  ...
  if (finishReason && finishReason !== 'STOP') {
    ...
    if (finishReason === 'MAX_TOKENS') {
      return { ok: false, error: 'O extrato é demasiado extenso para ser processado de uma vez (limite de output excedido). Tenta dividir o PDF em partes mais pequenas (ex: por trimestre).', status: 422 }
    }
    if (finishReason === 'SAFETY' || finishReason === 'RECITATION') {
      return { ok: false, error: 'O Gemini bloqueou este ficheiro por filtros de conteúdo. Tenta novamente ou usa outro formato (Excel/CSV).', status: 422 }
    }
    return { ok: false, error: `Processamento interrompido (${finishReason}). Tenta novamente.`, status: 422 }
  }
  ...
  if (!rawText.trim()) {
    ...
    return { ok: false, error: 'O Gemini não devolveu conteúdo para este ficheiro. Pode ser um PDF digitalizado (imagem) sem texto legível, ou estar protegido/corrompido.', status: 422 }
  }

  const clean = rawText.replace(/```(?:json)?[\r\n]*/g, '').replace(/```/g, '').trim()

  let transactions: any[] = []
  let meta: ParsedMeta = { saldo_final: null, iban: null, numero_conta: null, periodo_fim: null }
  ...
  if (parseFailed) {
    ...
    return { ok: false, error: 'A resposta do Gemini não veio em formato válido. Tenta novamente — por vezes ajuda repetir o pedido.', status: 422 }
  }
  ...
  return { ok: true, transactions: cleanTransactions, meta }
}
